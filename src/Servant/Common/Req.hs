...
type SupportsServantDisplay t m =
  (Reflex t, TriggerEvent t m, PerformEvent t m, HasWebView (Performable m), MonadIO (Performable m))

-- | This function actually performs the request.
performRequest :: forall t m uri. (SupportsServantDisplay t m)
               => Req t
               -> Dynamic t (URI.URIRef uri)
               -> Event t ()
...
performRequest req reqHost trigger = do
  ...
  let t :: Dynamic t [Either Text Text]
      ...
      baseUrl = fmap (first (T.pack . show) . TE.decodeUtf8' . URI.serializeURIRef') reqHost
...
performRequestNoBody :: forall t m uri. (SupportsServantDisplay t m)
                     => Req t
                     -> Dynamic t (URI.URIRef uri)
...
performRequestCT :: (SupportsServantDisplay t m,
                     MimeUnrender ct a)
                 => Proxy ct -> Req t -> Dynamic t (URI.URIRef uri)
